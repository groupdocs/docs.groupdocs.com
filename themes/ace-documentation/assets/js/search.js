...
SEARCH_LABELS.set('docs.groupdocs.com', 'groupdocsdocs');
SEARCH_LABELS.set('docs.groupdocs.cloud', 'groupdocsclouddocs');

function clearSearchResult(searchResults, pfName, sdk) {
    return searchResults.filter((el) => !el.host.includes("blog.groupdocs.com") && !el.host.includes("apireference") 
    && !el.host.includes("forum.groupdocs.com") && el.url.includes(`/${pfName}/`) && el.url.includes(`/${sdk}/`))
}
...
function determineSearchLabel() {
    return SEARCH_LABELS.has(window.location.hostname) ? SEARCH_LABELS.get(window.location.hostname) : SEARCH_LABELS.get('docs.groupdocs.com');
}

function getSearchResults(query, pfName, sdk) {
    var label = determineSearchLabel();
    var domain = label == "groupdocsdocs" ? "search.groupdocs.com" : "search.groupdocs.cloud";
    var category = `cat_groupdocs${pfName}`;
    $.ajax({
